...
<template>
  ...
  >
    <v-row>
      <v-col cols="6">
      <base-material-card height='319' class="pa-7">
         ...
         <v-card v-if="nearbyRoutesStopNum.length > 0">
            ...
            <br>
            <v-card-text v-for="(stops, index) in nearbyRoutesStopNum" :key='stops'>
               Stop No. {{ nearbyRoutesStopNum[index] }}. Bus routes at this stop: 
               <span v-html="nearbyRoutes[index]"></span>. Located at {{ nearbyRouteOnStreet[index] }} intersecting {{ nearbyRouteIntersectingStreet[index] }}
      ...
      >
        <base-material-card class="pa-7">
         ...
         <v-card v-if="allBusLoaded">
            ...
            <br>
            <v-card-text v-for="(est, index) in estArrival">The {{ bus[index] }} is estimated to arrive at {{ est }}</v-card-text>
         ...
         <v-card v-if="specificBusLoaded">
            ...
            <br>
            <v-card-text v-for="est in estArrival">The {{ bus }} is estimated to arrive at {{ est }}</v-card-text>
            <v-card-text><small>Disclaimer: Your bus may arrive 5-10 minutes early and will NOT wait at the stop to leave at the proper time, arrive early to avoid this common occurrence.</small></v-card-text>
         </v-card>
   ...
   <v-footer
      ...
      <v-col
         class="text-center"
         cols="12"
      >Data provided by:<a href='https://developer.translink.ca/' target="_blank">https://developer.translink.ca/</a>
      </v-col>
   </v-footer>
  </v-container>
</template>
            ...
            .then(coordinates => {
               ...
               .then(response => {
                  if (this.stopNumberNearMe.length > 0) {
                     let filteredNearbyStops = response.data.filter((data) => data['Routes'].includes(this.stopNumberNearMe))
